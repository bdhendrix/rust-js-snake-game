...
impl Game {
    ...
    fn process_movement(&mut self, timespan: f64, movement: Option<Movement>) {
        ...
        if movement.is_some() {
                ...
                !self.direction.equal_to(&new_direction)
            {
                ...
                {
                    let (old, old_rounded, new_rounded) = if rounded_x_changed {(old_x, old_x_rounded, new_x_rounded)} else {(old_y, old_y_rounded, new_y_rounded)};
    ...
    fn process_food(&mut self) {
        ...
        let head_segement = Segment::new(&self.snake[snake_len - 2], & self.snake[snake_len - 1]);

        if head_segement.is_point_inside(&self.food) {
